...
bool EvoWifi::doAPConnect(){
  ...
  if(WiFi.softAP(_SMT_HOST)){
    debugI("Config AP:%s IP:%s",WiFi.softAPSSID().c_str(),WiFi.softAPIP().toString().c_str());
    display.bootAPDisplay(WiFi.softAPSSID().c_str());
    dnsServer.start(53, "*", WiFi.softAPIP());
    evoWebserver.start();
    dnsTicker.attach_ms_scheduled(200,std::bind(&EvoWifi::loopEvent,this));
    return true;
  }
